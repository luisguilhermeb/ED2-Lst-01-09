...
int fatorBalanceamento_NO(ArvAVL no){
    return labs(altura_NO(no->esq) - altura_NO(no->dir));//retorna valor absoluto
}
  ...
  int res;
	...
	if((*raiz)->info == valor){
	    if(((*raiz)->esq == NULL || (*raiz)->dir == NULL)){// n� tem 1 filho ou nenhum
			...
			(*raiz)->info = procuraMenor((*raiz)->dir);
		...
		if (*raiz != NULL) (*raiz)->altura = maior(altura_NO((*raiz)->esq),altura_NO((*raiz)->dir)) + 1;
		return 1;
	}
